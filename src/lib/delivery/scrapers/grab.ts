// GrabExpress Instant scraper
// Uses: https://grab-express.vercel.app/api/instant?origin=depok%20sukmajaya&destination={address}
...
export class GrabExpressScraper {
  private readonly baseUrl = 'https://grab-express.vercel.app/api/instant';
  private readonly origin = 'depok sukmajaya';
  private readonly maxRetries = 2;

  /**
   * Scrape GrabExpress Instant pricing
   */
  async scrapePrice(address: string, cart: any[] = []): Promise<ScraperResult> {
    const cacheKey = CACHE_KEYS.GRAB(address);
    
    // Check cache first
    const cached = memoryCache.getCache(cacheKey);
    if (cached) {
      console.log(`[GrabExpress] Cache hit for ${address}`);
    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
        try {
          console.log(`[GrabExpress] Attempt ${attempt}/${this.maxRetries} for ${address}`);
          ...
          const response = await fetch(url, {
            ...
            headers: {
              ...
              'Referer': 'https://grab-express.vercel.app/'
            },
            // Add timeout
          ...
          const data: GrabResponse = await response.json();
          ...
          if (result) {
            ...
            console.log(`[GrabExpress] Successfully scraped ${address}: ${result.price}`);
            return result;
          }
          ...
          console.warn(`[GrabExpress] Attempt ${attempt} failed:`, error);
      ...
      console.error(`[GrabExpress] Scraping failed for ${address}:`, error);
      throw this.createError('grab', address, error);
    }
  }

  /**
   * Parse GrabExpress API response
   */
  private parseResponse(data: GrabResponse, address: string): ScraperResult | null {
    try {
      ...
      // Format 2: Instant object
      if (!price && data.instant) {
        if (typeof data.instant.price === 'number') {
          price = data.instant.price;
        }
        if (data.instant.eta) {
          eta = data.instant.eta;
        }
      }
      ...
      if (!price || isNaN(price) || price <= 0) {
        console.warn(`[GrabExpress] No valid price found in response for ${address}:`, data);
        return null;
      }
      ...
      console.error('[GrabExpress] Error parsing response:', error, data);
      return null;
    }
  }
...
export const grabExpressScraper = new GrabExpressScraper();